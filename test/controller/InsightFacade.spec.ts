...
describe("InsightFacade", function () {
	...
	describe("Add/Remove/List Dataset", function () {
		...
		it("8. should add one dataset with InsightDatasetKind==room", async function () {
			try {
				const result = await facade.addDataset("id", rooms, InsightDatasetKind.Rooms);
		...
		it("11. should reject when adding id of an already added dataset", function () {
			return facade
				.addDataset("id", validSections, InsightDatasetKind.Sections)
				.then((result) => {
		...
		it("26. should add datasets with valid course and valid room", async function () {
			try {
				const result = await facade.addDataset("id", validSections, InsightDatasetKind.Sections);
				// expect(result).to.deep.equal(["id"]);
				expect(result).to.be.instanceof(Array);
				expect(result.length).to.equal(1);
				expect(result).have.deep.members(["id"]);
				const result2 = await facade.addDataset("id2", rooms, InsightDatasetKind.Rooms);
				expect(result).to.be.instanceof(Array);
				expect(result2.length).to.equal(2);
				expect(result2).have.deep.members(["id", "id2"]);
			} catch (err) {
		...
		it("27. removed one dataset with room and return without error", async function () {
			try {
				const r = await facade.addDataset("id", rooms, InsightDatasetKind.Rooms);
				const r1 = await facade.removeDataset("id");
				const result = await facade.listDatasets();
				expect(result.length).to.equal(0);
			} catch (err) {
		...
		it("28. after adding a dataset with valid room, return the listDataset", async function () {
			try {
				const result = await facade.addDataset("id", rooms, InsightDatasetKind.Rooms);
				const res = await facade.listDatasets();
				expect(res).to.be.instanceof(Array);
